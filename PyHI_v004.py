...
class MainWindow(QMainWindow):
    ...
    def tab2_left_side_ctrl(self):
        ...
        for txt, pos in buttons.items():
            ...
            if txt == 'Clear' or txt == 'Undo':
                self.tab2_buttons[txt].setFixedWidth(75)
            else:
    ...
    def draw_tab1_fft(self):
        if self.power_spec_only == 1:
            img_rotation = self.img_rotation_chooser.value()
            self.current_img_fft_amp_rotated = ndimage.rotate(self.current_img_fft_amp, img_rotation, reshape=False)
        else:
