...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.threeDimgFileOpenButton = QAction(' &Open 3D Map', self)
        self.threeDimgFileOpenButton.triggered.connect(self.open_3D_map)
        self.fileMenu.addAction(self.threeDimgFileOpenButton)
        self.threeDimgFileOpenButton.setToolTip('''Open a standard helical map and generates 6 projections
    ...
    def open_3D_map(self):    
        self.power_spec_only = 0
        threeD_map_file_name = QFileDialog.getOpenFileName(self, 'choose 3D map', '.', 
                                                          filter="Map file (*.mrc)")
        if threeD_map_file_name[0] == '':
            return
        #try:
        with mrcfile.open(threeD_map_file_name[0]) as f:
            map_array = f.data
            if map_array.shape[0] != map_array.shape[1] or map_array.shape[0] != map_array.shape[2]:
                QMessageBox.information(self, "Alert", "Only cubic maps are supported\n")
                return
            try:
                y_pix = f.header.ny
                y_dim = f.header.cella.y
        ...
        self.mrc_data_array = np.empty((0,map_array.shape[0],map_array.shape[1])) 
        for i in range(0,360,60):
            map_array_rotated = ndimage.rotate(map_array, i, (1,2), reshape=False)
            map_array_rotated_proj = np.sum(map_array_rotated, axis=1)
            map_array_rotated_proj = map_array_rotated_proj.reshape((1,map_array.shape[0], map_array.shape[1]))
            self.mrc_data_array = np.vstack((self.mrc_data_array, map_array_rotated_proj))
        #except:
        #    QMessageBox.information(self, 'Error', 'Not a valid mrc map file?')
        #    return
        ...
        self.reset_tab1_display()
        self.reset_tab2_display()
        self.setWindowTitle(f'PyHI: {threeD_map_file_name[0]}')
    ...
    def draw_tab1_LL(self):
        ...
        try: 
            ...
            repeat_dist = (self.current_img_fft_amp.shape[-2]/self.LL_distance)*self.angpix
    ...
    def calc_LL_plot(self):
        try:
            ...
            if self.current_img_fft_phase.ndim == 3:
                slice_chosen = self.slice_chooser.value() - 1
                phase_data = self.current_img_fft_phase[slice_chosen, Y_phase, X_original_index[0]:X_original_index[-1]+1]
            else:
