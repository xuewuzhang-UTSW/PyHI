...
class MainWindow(QMainWindow):
    ...
    def tab1_ctrl_right_column(self):
        ...
        divider.setFrameShadow(QFrame.Sunken)
        layout.addWidget(divider, 2, 0, 1, 4) 
    ...
    def load_power_spec(self):
        ...
        self.img_xdim = self.current_img_fft_amp.shape[1]
        self.img_ydim = self.current_img_fft_amp.shape[0]
        self.origin[0] = self.img_xdim/2
        self.origin[1] = self.img_ydim/2
    ...
    def draw_2d_image(self):
        if self.mrc_data_array.ndim == 1:
            return
        self.current_img_array = self.mrc_data_array[self.current_slice]
        ...
        if self.img_shift != 0:
            self.current_img_array = ndimage.shift(self.current_img_array, [0, self.img_shift])
        ...
        if self.twoD_img_shown == None:
            ...
            self.ax2d.set_xlim(0, self.img_xdim)
            self.ax2d.set_ylim(0, self.img_ydim)
        ...
        if self.midpoint_of_2dimage == None:
            self.midpoint_of_2dimage=(self.ax2d.plot(
                [self.img_xdim/2, self.img_xdim/2], [0, self.img_ydim], '-', color='b', markersize=5, lw=0.5))[0]
        else:
            self.midpoint_of_2dimage.set_data([self.img_xdim/2, self.img_xdim/2], [0, self.img_ydim])
    ...
    def set_img_shift(self):
        self.img_shift = self.img_shift_chooser.value()
        if self.mrc_data_array is not None:
            self.draw_2d_image()
        if self.current_img_fft_amp is not None:
            self.draw_tab1_fft()
    ...
    def draw_tab1_fft(self):
        ...
        if self.tab1_fft_shown is None:
            ...
            self.tab1_axfft.set_xlim(0, self.img_xdim)
            self.tab1_axfft.set_ylim(0, self.img_ydim)
    ...
    def draw_tab2_fft(self):
        try:
            ...
            if self.tab2_fft_shown is None: 
                ...
                self.tab2_axfft.set_xlim(0, self.img_xdim)
                self.tab2_axfft.set_ylim(0, self.img_ydim)
    ...
    def button_push_connect(self):
        ...
        self.img_shift_chooser.valueChanged.connect(self.set_img_shift)
